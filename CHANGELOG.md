...
### 2026-09-09：基于历史复评的MD轻量优化

- 保留2.1.0-rc.2版本标记；本节为未发布改进，不宣称论文分数已提高。
- 在现有研究契约核对题目与材料，主要分析后只选当前方向2—3个关键反例检查；不新增运行阶段、文件合同或评分器。
- 正文整合合并重复限制、删除过程旁白；不改默认字数，不以空话补齐。
- 审图先读最终图片关系再对照正文；保留真实生图、中文覆盖、领域精确图和实际嵌入要求。
- 导出后核对实际摘要、目录、章序、公式、表格及图片页面；排除正常页眉重复。
- 弱模型只对实际错误增加局部示例；强模型保持自主判断。纠正真实内部再分析被一律降为方案的过度规则。
- 未新增或修改Python脚本；仅复用既有编译与验证工具。真实新稿改善仍待用户直接测试。

### 2026-09-07：整体主分支同步与直接评分

- 将当前2.1.0-rc.2整体同步主分支，保留实际版本标记。
- 测试采用升级后生成当前版本论文、逐篇评分；不强制旧版A/B或固定批次阻塞提交。
- Grok十九方向新批次进入独立审阅，尚未发布内容评分结论。
- 历史测试控制器已发现跨进程状态覆盖及A/B汇总残留，当前以原始论文目录与单篇记录为评审输入。
...
- GitHub Actions在Python 3.9、3.11和Node 22上完成候选工程回归。
- 新增三方向18次小试与30次完整盲评协议；真实论文A/B仍待执行。

## 2.1.0-rc.1 - 2026-09-02

- 写作运行时彻底移除数字评分、作者自称独立审稿和QUALITY第五裁决；权威状态只读取证据、图片、公式与交付四类机械报告。
- 新增无评分`qa-observations.json`投影和写作流外`eval/build_review_package.py`；独立评分不能修改权威交付状态。
- 能力输入改为true/false/null三态，null不再被误判为无工具；19方向内置DEFAULT_FEATURES，模型只填写必要增删。
- 新增安全`paper.py prepare --amend`：生成版本化提示词、保留旧摘要、阻止移除已有产物模块，并使受影响阶段失效。
- Full/Compact由同一方向与公共源的CORE段生成；保守参数下19方向弱模型最终输入均不超过15KB。
- 19个方向全部扩充专业方法、证据边界、常见错误和结论条件；构建校验强制专业内容占最终方向提示词至少30%。
