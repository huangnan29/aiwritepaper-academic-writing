...
### 当前未发布的轻量改进（2026-09-09）

历史同题复评显示，Grok Build的1.9.1十九篇按统一口径约79.9分，当前2.1.0-rc.2十九篇约77.4分；旧87.2分口径偏宽，不能直接拿来衡量升级落差。这是单一评审者的抽查评分，不是版本因果试验或各行业达90分的证明。

本次主要改Markdown：在已有契约核对题目与材料，检查本题少量专业反例，合并重复边界声明，核对最终图片关系与实际导出页面。**没有新增或修改Python脚本，也没有新增强制中间文件。** 仍是一次准备、一份执行MD、既有机械检查；强模型自主执行，弱模型仅在实际出错时增加局部辅助。新稿效果尚待直接测试，不要求重跑旧版A/B。
...
当前评测不再要求重跑旧版、完成18/30任务矩阵或频繁切换仓库。`eval/ab_runner.py`并行补充Grok 4.6与Gemini 3.8 Flash各两篇；每篇使用独立目录、日志和状态文件。使用方式见[评测说明](eval/README.md)。Antigravity固定使用真实`agy` CLI与`gemini-3.8-flash-high`，不以Gemini CLI代替。
