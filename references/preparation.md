...
| capabilities | 四项true/false/null能力；null表示未知，不能当成不可用 |
| capability_details | 可选tool、caller与evidence；不含账号令牌或Cookie |
| run_mode | 默认FULL_BUILD；其余按用户任务范围 |
| feature_overrides | 方向自带默认features；模型只记录add/remove |
...
| title_policy | STRICT、ASK或SAFE_DOWNGRADE；默认ASK |
...
使用[示例](paper-request.example.json)时移除example_only并填写真实任务。能力未知保留null，不能为了完成准备写false。工具记录生成时间，但不会联网探测、登录账户或按模型品牌猜能力。
...
发现漏模块时创建符合`schemas/prompt-amendment.schema.json`的prompt-amendment.json，运行`paper.py prepare --amend`。它生成版本化新提示词并保留旧版；已有产物的模块不能移除，正文开始后相关阶段会失效并须重检。
...
必要时增加--claim-level明确研究性质。已有路径不必重复指定；--plan只显示计划。qa-observations.json存在时自动派生主张、图片、页面和问题视图；它只绑定真实观察，不创建分数或审稿身份。
...
完整版与紧凑版由同一源生成。compact只取明确CORE段并加载同一真实性底线；最终输入预算为15KB，超限时构建校验失败。数字评分只允许在运行时之外的eval冻结包上进行。
