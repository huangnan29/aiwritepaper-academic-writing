...
本文件是无Python时的旧兼容回退，不具备新task-selection模块筛选。能够运行Python时优先使用compose_prompt.py --task-selection；不可运行时在run-params.md明确当前模式与禁止扩大范围，记录未裁剪及权威检查能力缺口，不能宣称已执行新模块筛选。不得让模型手抄或总结规则来模拟确定性合成。
