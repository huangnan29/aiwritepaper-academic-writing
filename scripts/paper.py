...
def capability_report(request: dict, adapter: str) -> dict:
    observed = nonempty(request.get("observed_at"), "observed_at")
    try:
    ...
    if not isinstance(source, dict):
        raise ValueError("缺少实际capabilities，不能把未检查写成无能力")
    result = {"schema_version": "1.0", "agent_adapter": adapter, "observed_at": observed}
    for name in CAPABILITIES:
        item = source.get(name)
        if not isinstance(item, dict) or not isinstance(item.get("available"), bool):
            raise ValueError(f"capabilities.{name}缺少实际available观察")
        for field in ["tools", "callers"]:
            if not isinstance(item.get(field), list) or any(not isinstance(x, str) or not x.strip() for x in item[field]):
                raise ValueError(f"{name}.{field}必须是字符串列表")
        if set(item["callers"]) - CALLERS:
            raise ValueError(f"{name}.callers含未知调用层")
        nonempty(item.get("evidence"), f"{name}.evidence")
        if item["available"] and (not item["tools"] or not item["callers"]):
            raise ValueError(f"{name}可用必须有工具及调用层依据")
...
def prepare(root: Path, request_file: Path, preview: bool = False) -> dict:
    ...
    params = {"RUN_MODE": mode, "OUTPUT_DIR": str(root), "MODEL_LABEL": model, "RUN_LABEL": request.get("run_label", root.name), "DIRECTION_ID": direction,
              ...
              "EXECUTION_PROFILE": profile, "TARGET_LENGTH": lengths["target"], "MIN_LENGTH": lengths["minimum"], "MAX_LENGTH": lengths["maximum"]}
    ...
    task = {"schema_version": "1.0", "run_mode": mode, "direction_id": direction, "features": request.get("features")}
    ...
    if profile != "FULL_AUTONOMY":
        ...
        rules = SKILL_ROOT / "references/profiles" / ("guided.md" if profile == "GUIDED" else "weak-model.md")
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--request", type=Path, default=Path("paper-request.json"))
    parser.add_argument("--preview", action="store_true")
    args = parser.parse_args(argv)
    try:
        request = args.request if args.request.is_absolute() else args.root / args.request
        result = prepare(args.root, request, args.preview)
