...
from build_compiled import SKILL_ROOT, COMMON_DIR, COMMON_FILES, COMPILED_DIR, COMPACT_DIR, direction_files, render_compiled, render_compact
...
def main():
    ...
    for folder, suffix, renderer in [(COMPILED_DIR, "full", render_compiled), (COMPACT_DIR, "compact", render_compact)]:
        ...
        for direction in directions:
            ...
            if not target.is_file() or target.read_text(encoding="utf-8") != renderer(direction):
                ...
                errors.append(f"兼容提示词超出50KB维护预算：{target.name}")
    for direction in directions:
        text = render_compiled(direction)
        blocks = dict(MODULE_PATTERN.findall(text))
        for name in COMMON_FILES:
            source = (COMMON_DIR / name).read_text(encoding="utf-8").strip()
            if text.count(source) != 1 or Path(name).stem not in blocks:
                errors.append(f"公共规则非单一来源：{direction.name}/{name}")
        if direction.read_text(encoding="utf-8").strip() not in blocks.get("direction", ""):
            errors.append(f"方向内容丢失：{direction.name}")
        if set(blocks) != {Path(n).stem for n in COMMON_FILES} | {"direction", "rubric", "method"}:
            errors.append(f"模块边界不完整：{direction.name}")
        compact = render_compact(direction).replace("紧凑兼容论文生成提示词", "完整论文生成提示词", 1)
        if compact != text:
            errors.append(f"强弱规则漂移：{direction.name}")
    ...
    for script in ["paper.py", "check_paper.py", "compose_prompt.py", "select_execution_profile.py", "prepare_audit_views.py", "prepare_resume.py", "compose_revision.py", "resolve_default_length.py", "verify_evidence_integrity.py", "verify_figure_package.py", "verify_formula_rendering.py", "verify_manuscript_delivery.py", "verify_quality_package.py", "capture_provenance.py", "adjudicate_status.py", "write_skipped_report.py", "render_svg_layout.mjs"]:
    ...
    if errors:
        ...
        return 1
    print("通过：19方向full/compact同源、模式组合、版本和必要功能完整；不代表论文质量达标。")
    return 0
