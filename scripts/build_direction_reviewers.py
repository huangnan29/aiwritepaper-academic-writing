...
def main():
 d=json.loads((ROOT/"references/quality/direction-rubrics.json").read_text(encoding="utf-8"));g=json.loads((ROOT/"references/quality/direction-method-gates.json").read_text(encoding="utf-8"))["directions"];out=ROOT/"references/reviewers";out.mkdir(exist_ok=True)
 for key,r in d["directions"].items():
  focus="\n".join(f"- {x}" for x in r["focus"]);crit="\n".join(f"- {x}" for x in r["critical"])
  gates="\n".join(f"- {x}" for x in g[key]["completion_gates"]);downgrade=g[key]["retitle_or_downgrade"]
