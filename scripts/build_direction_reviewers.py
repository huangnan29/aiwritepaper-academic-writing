...
def main():
    ...
    gates = json.loads((ROOT / "references/quality/direction-method-gates.json").read_text(encoding="utf-8"))["directions"]
    out = ROOT / "references/reviewers"
    out.mkdir(exist_ok=True)
    for key, rubric in rubrics["directions"].items():
        focus = "\n".join(f"- {x}" for x in rubric["focus"])
...
材料不足时：{gates[key]["retitle_or_downgrade"]}
